$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν μπορείτε να εκδώσετε αρχικό δελτίο ταυτότητας."
        : "We are sorry but you are not entitled to issue an original ID card";
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να εκδόσετε το δελτίο ταυτότητας είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive your ID card are the following:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρεπει να έχετε Ελληνική υπηκοότητα και να έχετε συμπληρώσει το δωδέκατο έτος της ηλικίας σας.") : skipToEnd("You must have Greek citizenship and be at least 12 years of age.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Αυτή η διαδικασία αφορά μόνο στην αρχική έκδοση δελτίου ταυτότητας.") : skipToEnd("This procedure only concerns the initial issuance of an identity card.");
      }
      else if (currentQuestion === 5 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Ειναι υποχρεωτική η αυτοπρόσωπη παρουσία στην αστυνομική αρχή. Σε περίπτωση αδυναμίας επικοινωνήστε με το οικείο Τμήμα Ασφαλείας ή Αστυνομικό τμήμα για να ενημερωθείτε για την διαδικασία.") : skipToEnd("It is mandatory to appear in person at the police station. If you are unable to do so, please contact the relevant Security Department or Police Department to be informed about the procedure.");
      }
      else {
